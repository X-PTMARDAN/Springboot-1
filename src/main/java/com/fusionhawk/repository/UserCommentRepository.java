...
public interface UserCommentRepository extends JpaRepository<UserCommentsRes, String> {
	...
	String fetchUserCommentsQuery_monthly = "SELECT calendar_yearmonth As Calendar_Week, comments1 As comments2 FROM plan_data WHERE cpg IN (:cpgList) AND plant IN (:plantList) AND Calendar_Week BETWEEN :startWeek AND :endWeek AND Sku IN (SELECT DISTINCT(Name) from TABLE_NAME where ForecastingGroup IN (:forecastingGroupList)) AND comments1!='comment1'" ;

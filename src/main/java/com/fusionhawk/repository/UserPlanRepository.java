...
public interface UserPlanRepository extends JpaRepository<UserPlanRes, String> {
	...
	String fetchUserPlanQuery = "SELECT Calendar_Week, SUM(fva) AS fva, SUM(Final_Forecast) AS Final_Forecast FROM suvid_plan\n" + 
			"WHERE cpg IN (:cpgList) \n" + 
			"AND plant IN (:plantList) \n" + 
			"AND Calendar_Week BETWEEN :startWeek AND :endWeek \n" + 
			"AND Sku IN (SELECT DISTINCT(Name) From FINAL_AURORA_UPDATED where ForecastingGroup IN (:forecastingGroupList)) AND Final_Forecast!=0 GROUP BY Calendar_Week";
